    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                ...
                <a href="{{ route('admin.slider.create') }}" class="btn btn-primary btn-sm float-right">Add New</a>
            </div>
            <div class="card-body">
                ...
                <table id="bootstrap-data-table" class="table table-striped table-bordered text-center">
                    ...
                    <tbody>
                        @foreach ($sliders as $sk => $slider)
                        <tr>
                            <td>{{ ++$sk }}</td>
                            <td>
                                @if ($slider->image)
                                    <img src="{{ asset($slider->image) }}" height="50" alt="">
                                @else
                                    <span class="text-danger">---</span>
                                @endif
                            </td>
                            <td>{{ !empty($slider->heading) ? $slider->heading : '---' }}</td>
                            <td>
                                <div class="btn-group">
                                    <a href="{{ route('admin.slider.edit', $slider->id) }}" class="btn btn-info btn-sm">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                    <form action="{{ route('admin.slider.delete') }}" method="post" class="delete_form">
                                        @csrf
                                        @method('DELETE')
